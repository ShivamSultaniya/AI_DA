...
def sample_from_distribution(prob_table):
    """Sample a value from a probability table."""
    rnd = random.random()
    cumulative = 0
...
def monte_carlo_inference(query_node, evidence, num_samples=10000):
    """
    Perform Monte Carlo inference to estimate P(query_node | evidence).
    
    :param query_node: Node for which we want to estimate the probability.
    :param evidence: Dictionary specifying evidence (e.g., {'A': 'yes'}).
    :param num_samples: Number of samples to generate.
    :return: Estimated probabilities for the query_node values.
    """
    matching_samples = []
